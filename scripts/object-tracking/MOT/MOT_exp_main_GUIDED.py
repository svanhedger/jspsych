...
from psychopy.constants import NOT_STARTED, STARTED, FINISHED, PLAYING
import math
...
class MOTobj:
    def __init__(self, default_color=WHITE):
        ...
        self.dx, self.dy = choice([dx for dx in range(min_spd, max_spd) if dx not in [0]])*2, \
                           choice([dy for dy in range(min_spd, max_spd) if dy not in [0]])*2
...
def real_trials(master_list, distractor_list, target_list, CRT, recorder):
    ...
    pautone = ["pau.wav","to.wav","ne.wav"] # each word is a list of syllables
    gamilu =["ga.wav","mi.wav","lu.wav"]
    nurafi=["nu.wav","ra.wav","fi.wav"]
    maipuki=["mai.wav","pu.wav","ki.wav"]
    ...
    trialwords=math.floor((Tani-Tfl)*1.06)  # amount of words per trial defined by the time duration of the circles moving
    expwords=trialwords*n_real/4 # amount of times each different word repeats itself
    volumeWords=0.015625 #set the volume
    mySound = sound.Sound(value=words[word][syllable]) #loading the first syllable of the first word of the words list shuffled
    ...
    while True:
        ...
        if completed_practice_trial_count < n_real:
                ...
                if dt <= Tfix:
                    ...
                    if expwords>0 and trialwords>0: #
                        if mySound.status == NOT_STARTED: 
                            mySound = sound.Sound(value=words[word][syllable], volume=volumeWords) #play the first Syllable
                            mySound.play()
                        ...
                        if mySound.status == FINISHED:    #play each syllable
                            if trialwords<3: #the last 2 words of each trial
                                volumeWords/=2 #decreases the volume by half in each syllable
                            if trialwords>math.floor((Tani-Tfl)*1.06-2): #the first 2 words of each trial
                                volumeWords*=2 #increases the volume of each syllable by duplicating it
                            syllable+=1     # next syllable
                            mySound = sound.Sound(value=words[word][syllable], volume=volumeWords) #load the audio file
                            mySound.play()
            ...
            if reset:
                ...
                trialwords=math.floor((Tani-Tfl)*1.06) # resets amount of words per trial
