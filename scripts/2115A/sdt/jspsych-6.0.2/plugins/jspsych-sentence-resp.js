...
jsPsych.plugins['sentence-resp'] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    for (var i = 0; i < trial.questions.length; i++) {
      ...
      if(trial.questions[i].rows == 1){
        html += '<input type="text" autofocus name="#jspsych-survey-text-response-' + i + '" size="'+trial.questions[i].columns+'" value="'+trial.questions[i].value+'"></input>';
      } else {
        html += '<textarea maxlength = "1" autofocus name="#jspsych-survey-text-response-' + i + '" cols="' + trial.questions[i].columns + '" rows="' + trial.questions[i].rows + '">'+trial.questions[i].value+'</textarea>';
      }
      html += '</div>';
    }
