...
task. Participants complete runs (30 scored trials) in an adaptive fashion.
If performance on the run is good, the n-level increases. If performance is middling,
the n-level stays the same. If performance is poor, the n-level decreases.
...
var NB_SEQUENCE = [1]; // critical - this specifies where you should start
var currentN = 1; //critical - this specifies where you should start
var numRuns = 10; //how many runs do you want to go through in the expt
var NB_INDEX = 0; //selection index for nb sequence
var TRIAL_INDEX; //selection index for the trials within an n-back run
var run_sequence;

var HIT = 0; //counter of total hits for the run
var FA = 0; //counter of total false alarms for 1-back
...
//looping function to repeat the trial for the length of the stim array (30 + n)
	...
	var shortBreak = {
		...
		on_finish: function(){
			if((HIT-FA) >= 5){
				currentN += 1; //increase the N for great performance
				var statusN = '+1'
			} else if ((HIT-FA) < 5 && (HIT-FA) >= 2){
				currentN = currentN; //unnecessary but oh well
				var statusN = '0'
			} else {
				currentN -= 1; //decrease for poor performance
				var statusN = '-1'
				if(currentN < 1){
					currentN = 1; //floor is 1-back
				}
			}
			var test_console = 'hits = '+HIT+'... false alarms = '+FA+'... status ='+statusN;
			console.log(test_console);

			var sumData = {
			designation: 'n-back-run-summary',
			STATUS: statusN,
			N: NB_SEQUENCE[NB_INDEX],
			HIT: HIT,
			FA: FA
				};
		 jsPsych.data.addDataToLastTrial(sumData);

		 NB_SEQUENCE.push(currentN); //push the current N to the trial run list
		 NB_INDEX += 1; //this advances the index to search for the next 'n' level in NB_SEQUENCE
		 HIT = 0; //reset the hit counter
		 FA = 0; //reset the fa counter
		}
    };
